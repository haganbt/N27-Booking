...
<?php
    if ( wrap_session_is_registered("admin_user") ) {
        ?><td align="right" class="SectionHeaderStyle" style="text-align: right;"><a href="<?= href_link(FILENAME_DAY_VIEW, 'date='.SELECTED_DATE.'&print_view=1&'.make_hidden_fields_workstring(array('view', 'loc')), 'NONSSL')?>" target="_blank">print</a></td><?php
    }
?>
  </tr>
</table>
...
<table cellspacing="1" cellpadding="1" width="100%" border="0">
  ...
  <tr>
	...
	<td class="BgcolorBright" align="center" valign="middle">
		<a href="<?=href_link(FILENAME_DAY_VIEW, 'date='.PREVIOUS_DAY_DATE.'&'.make_hidden_fields_workstring(array('view', 'loc')), 'NONSSL')?>"><img
		src="<?=DIR_WS_IMAGES?>/prev.gif"
		alt="Previous Day" /></a><b><?=SELECTED_DATE_LONGSTR?></b><a
		href="<?=href_link(FILENAME_DAY_VIEW, 'date='.NEXT_DAY_DATE.'&'.make_hidden_fields_workstring(array('view', 'loc')), 'NONSSL')?>"><img
		src="<?=DIR_WS_IMAGES?>/next.gif" alt="Next Day" /></a></td>
  </tr>


<?
  $currentUsersID = get_user_id( $_SESSION['valid_user'] ) ;
  $count = 0;
  $width_length = 5;
...
?>
	<tr>
	<td align="center" valign="top" width="<?=$time_cell_width?>"
...
<?
    ...
    if ( wrap_session_is_registered("admin_user") || ( mktime( $hour, $min, $sec, $month, $day, $year ) < $booking_limited_time ) ) {
        $make_booking_link = href_link(FILENAME_ADD_EVENT, 'start_time='.$std_time_str.'&'.make_hidden_fields_workstring(array('date', 'view', 'loc')), 'NONSSL') ;
    } else {
	...
	if (ALLOW_ADDITIONS_FLAG) {
	    //Modified by MJ on 10/03/05 - if the slot is already booked then don't create a link to add for this time slot!
	    if ( (strlen($event_row_data[$display_time]) > 1) || ($event_row_data[$display_time] == '0' ) ) {
	        echo $time_str ;
	    } else {
            echo '<a href="' . $make_booking_link . '">' . $time_str . '</a>' ;
        }
        echo ' &nbsp; <br /> <br />' ;
	} else {
?>
...
<?
	...
	if (strlen($event_row_data[$display_time]) > 1) {
		...
		//make exception for their own bookings, all users should always be able to see those!
...
?>
		...
		class="FontSoftSmall">&nbsp;<a href="<?=$event_url?>" onmouseover="return overlib('' +
		'<?=overlib_escape(htmlentities($over_text, ENT_QUOTES, 'ISO-8859-1'))?>' +
		' ', CAPTION, 'Event Time: <?=$start_time?>-<?=$end_time?>');"
		onmouseout="nd();"><?=$start_time?>-<?=$end_time?></a>&nbsp; -
		Subject: <?=$this_event['subject']?>
		<?php
		if ( $numBookingOptions > 0 ) {
            echo ' (' ;
            for ( $o = 0 ; $o < $numBookingOptions ; $o++ ) {
